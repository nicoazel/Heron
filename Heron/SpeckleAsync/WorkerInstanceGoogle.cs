...
/// </summary>
public abstract class WorkerInstanceGoogle<T>(T parent, string id, CancellationToken cancellationToken)
    where T : GH_Component
{
    ...
    /// <returns></returns>
    public abstract WorkerInstanceGoogle<T> Duplicate(string id, CancellationToken cancellationToken);

    /// <summary>
    /// This method is where the actual calculation/computation/heavy lifting should be done.
    /// <b>Make sure you always check as frequently as you can if <see cref="WorkerInstanceGoogle{T}.CancellationToken"/> is cancelled. For an example, see the PrimeCalculatorWorker example.</b>
    /// </summary>
    /// <remarks>
