...
namespace GrasshopperAsyncComponent.Google;

// Replace 'required' with explicit property initialization in the Worker<T> class

internal sealed class Worker<T> : IDisposable
    where T : GH_Component
{
    public WorkerInstanceGoogle<T> Instance { get; set; }
...
/// </summary>
public abstract class GH_AsyncComponentGoogle<T> : GH_Component, IDisposable
    where T : GH_Component
{
    ...
    public IEnumerable<WorkerInstanceGoogle<T>> Workers => _workers.Select(x => x.Instance);
    ...
    /// </summary>
    public WorkerInstanceGoogle<T>? BaseWorker { get; set; }
    ...
    protected GH_AsyncComponentGoogle(string name, string nickname, string description, string category, string subCategory)
